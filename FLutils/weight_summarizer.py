...
class WeightSummarizer:
    ...
    def process(self,
                ...
                density_distribution: Optional[List[np.ndarray]] = None) -> List[np.ndarray]:
...
class FedAvg(WeightSummarizer):
    ...
    def process(self,
                client_weight_list: List[List[np.ndarray]],
                global_weights_per_layer: Optional[List[np.ndarray]] = None,
                density_distribution: Optional[List[np.ndarray]] = None) -> List[np.ndarray]:
        ...
        for layer_index in range(len(weights_average)):
            ...
            for client_weight_index in range(nb_clients):
                ...
                # this step can be done at client side (client can send the difference of the weights)
...
class FedBoost(WeightSummarizer):
    ...
    def process(self,
                client_weight_list: List[List[np.ndarray]],
                global_weights_per_layer: Optional[List[np.ndarray]] = None,
                density_distribution: Optional[List] = None) -> List[np.ndarray]:
        nb_clients = len(client_weight_list)
        weights_boost = [np.zeros_like(w) for w in client_weight_list[0]]
        for layer_index in range(len(weights_boost)):
            w = weights_boost[layer_index]
            if global_weights_per_layer is not None:
                global_weight_mtx = global_weights_per_layer[layer_index]
            else:
                global_weight_mtx = np.zeros_like(w)
            for client_weight_index in range(nb_clients):
                client_weight_mtx = client_weight_list[client_weight_index][layer_index]
                client_weight_diff_mtx = client_weight_mtx - global_weight_mtx
                w += client_weight_diff_mtx*density_distribution[client_weight_index]
            weights_boost[layer_index] =  w + global_weight_mtx
        return weights_boost
